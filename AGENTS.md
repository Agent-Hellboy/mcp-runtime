...
| Agent adapters | `internal/cli/adapter/`, `internal/agentadapter/`, `services/runtime-control/internal/runtimeapi/adapter.go` | `adapter proxy/stdio` → `POST /api/v1/runtime/adapter/sessions`; grants enforced on platform |
...
| Sentinel services | `services/platform-api`, `services/runtime-control`, `services/analytics-api`, `services/ui`, `services/ingest`, `services/processor`, `services/mcp-gateway`, … | Separate `go.mod` where present; Go 1.26 for shared imports |
