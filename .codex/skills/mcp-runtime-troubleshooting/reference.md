...
- **mtls server unreachable through ingress on k3s (connection timeout at the gateway):** the operator's `<server>-mtls-gateway` NetworkPolicy admits only the ingress controller pods. Setup detects the running Traefik and passes `MCP_INGRESS_CONTROLLER_NAMESPACE`, `MCP_INGRESS_CONTROLLER_SERVICE_ACCOUNT` and `MCP_INGRESS_CONTROLLER_POD_LABELS` to the operator. Without them it assumes repo-managed `traefik/traefik` with `app=traefik`, which does not match k3s (`kube-system`, `app.kubernetes.io/name=traefik`). Check with `kubectl -n mcp-runtime get deploy mcp-runtime-operator-controller-manager -o jsonpath='{.spec.template.spec.containers[0].env}'`, then `kubectl get networkpolicy <server>-mtls-gateway -o yaml`; rerun setup (or set the env on the operator) to fix. The pinned `TRUSTED_PROXY_SPIFFE_ID` follows the same values.
