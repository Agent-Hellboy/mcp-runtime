...
func (c *Client) InsertEvents(ctx context.Context, batch []events.Envelope) error {
	...
	insert, err := c.Conn.PrepareBatch(ctx, "INSERT INTO "+c.DBName+".events (timestamp, source, event_type, payload)")
	if err != nil {
	...
	for _, event := range batch {
		if err := insert.Append(event.TimestampTime(now), event.Source, event.EventType, string(event.Payload)); err != nil {
