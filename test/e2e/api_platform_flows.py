...
expect_status(f"{api_base}/api/v1/auth/login", 401, method="POST", body={"email": f"missing-{suffix}@mcpruntime.org", "password": "wrong-password"})
oidc_status, _, oidc_body = request(f"{api_base}/api/v1/auth/oidc", method="POST", body={})
...
expect_status(f"{api_base}/api/v1/auth/signup", 400, method="POST", body={"email": f"bad-role-{suffix}@mcpruntime.org", "password": test_user_password, "role": "root"})
expect_status(f"{api_base}/api/v1/auth/signup", 403, method="POST", body={"email": f"admin-denied-{suffix}@mcpruntime.org", "password": test_user_password, "role": "admin"})

admin_login = expect_json(f"{api_base}/api/v1/auth/login", method="POST", body={"email": platform_admin_email, "password": platform_admin_password})
...
signup = expect_json(f"{api_base}/api/v1/auth/signup", status=201, method="POST", body={"email": signup_email, "password": test_user_password})
...
created_user_key = expect_json(f"{api_base}/api/v1/user/api-keys", method="POST", headers=user_headers, body={"name": f"e2e-key-{suffix}"})
...
created_credential = expect_json(f"{api_base}/api/v1/user/registry-credentials", status=201, method="POST", headers=user_headers, body={"name": f"e2e-registry-{suffix}"})
...
team_created = expect_json(f"{api_base}/api/v1/runtime/teams", method="POST", headers=admin_headers, body={"slug": team_slug, "name": f"E2E API {suffix}"})
...
expect_json(f"{api_base}/api/v1/runtime/teams/{quote_segment(team_slug)}", headers=admin_headers)
expect_json(f"{api_base}/api/v1/runtime/teams/{quote_segment(team_slug)}/members", headers=admin_headers)
...
team_user = expect_json(f"{api_base}/api/v1/users", status=201, method="POST", headers=admin_headers, body={"email": team_user_email, "password": test_user_password, "role": "user"})
...
expect_json(f"{api_base}/api/v1/runtime/teams/{quote_segment(team_slug)}/members/{quote_segment(team_user_id)}", method="DELETE", headers=admin_headers)

namespaces = expect_json(f"{api_base}/api/v1/runtime/namespaces", headers=admin_headers)
...
namespace_item = expect_json(f"{api_base}/api/v1/runtime/namespaces/{quote_segment(team_namespace)}", headers=admin_headers)
...
expect_status(f"{api_base}/api/v1/registry/authz", 401, headers={"X-Forwarded-Uri": "/v2/_catalog"})
expect_status(f"{api_base}/api/v1/registry/authz", 204, headers=merged_headers(admin_key_headers, {"X-Forwarded-Uri": f"/v2/{team_slug}/demo/manifests/latest"}))
personal_scope = user_namespace or registry_username
if personal_scope:
    expect_status(f"{api_base}/api/v1/registry/authz", 403, headers=merged_headers(user_key_headers, {"X-Forwarded-Uri": f"/v2/{personal_scope}/demo/manifests/latest"}))
registry_basic_headers = basic_headers(registry_username, registry_password)
expect_status(f"{api_base}/api/v1/registry/authz", 204, headers=merged_headers(registry_basic_headers, {"X-Forwarded-Uri": f"/v2/{team_slug}/demo/manifests/latest"}))
expect_status(f"{api_base}/api/v1/registry/authz", 204, headers=merged_headers(registry_basic_headers, {"X-Forwarded-Uri": f"/v2/{team_namespace}/demo/manifests/latest"}))

expect_json(f"{api_base}/api/v1/analytics/usage?limit=3", headers=admin_key_headers)
expect_json(f"{api_base}/api/v1/user/analytics/usage?limit=3", headers=user_headers)
expect_json(f"{api_base}/api/v1/user/activity/image-publish", status=202, method="POST", headers=user_headers, body={
...
server_item = expect_json(f"{api_base}/api/v1/runtime/servers/mcp-servers/{quote_segment(server_name)}", headers=admin_key_headers)
...
server_events = expect_json(f"{api_base}/api/v1/runtime/server-events?namespace=mcp-servers&server={urllib.parse.quote(server_name)}&limit=5", headers=admin_key_headers)
...
expect_json(f"{api_base}/api/v1/runtime/grants", method="POST", headers=admin_key_headers, body={
...
expect_json(f"{api_base}/api/v1/runtime/sessions", method="POST", headers=admin_key_headers, body={
...
expect_json(f"{api_base}/api/v1/runtime/grants/mcp-servers/{quote_segment(api_runtime_grant)}", headers=admin_key_headers)
expect_json(f"{api_base}/api/v1/runtime/sessions/mcp-servers/{quote_segment(api_runtime_session)}", headers=admin_key_headers)
expect_json(f"{api_base}/api/v1/runtime/sessions/mcp-servers/{quote_segment(api_runtime_session)}", method="DELETE", headers=admin_key_headers)
expect_json(f"{api_base}/api/v1/runtime/grants/mcp-servers/{quote_segment(api_runtime_grant)}", method="DELETE", headers=admin_key_headers)

temp_server_name = f"e2e-api-server-{suffix}"
temp_server = expect_json(f"{api_base}/api/v1/runtime/servers", method="POST", headers=admin_headers, body={
...
expect_json(f"{api_base}/api/v1/runtime/servers/mcp-servers/{quote_segment(temp_server_name)}", headers=admin_headers)
expect_json(f"{api_base}/api/v1/runtime/servers/mcp-servers/{quote_segment(temp_server_name)}", method="DELETE", headers=admin_headers)

deployment_name = f"e2e-api-deploy-{suffix}"
expect_json(f"{api_base}/api/v1/deployments?namespace={quote_segment(team_namespace)}", headers=admin_headers)
deployment = expect_json_retry(f"{api_base}/api/v1/deployments", method="POST", headers=admin_headers, body={
...
expect_json(f"{api_base}/api/v1/admin/deployments?namespace={quote_segment(team_namespace)}", headers=admin_headers)
expect_json(f"{api_base}/api/v1/deployments/{quote_segment(team_namespace)}/{quote_segment(deployment_name)}", method="DELETE", headers=admin_headers)

admin_namespaces = expect_json(f"{api_base}/api/v1/admin/namespaces", headers=admin_headers)
...
admin_audit = expect_json(f"{api_base}/api/v1/admin/audit?limit=5", headers=admin_headers)
check("audit_logs" in admin_audit, "GET /api/admin/audit returned audit logs", f"admin audit response: {admin_audit}")
admin_operations = expect_json(f"{api_base}/api/v1/admin/operations?limit=5", headers=admin_headers)
...
expect_json(f"{api_base}/api/v1/user/registry-credentials/{quote_segment(credential_id)}", method="DELETE", headers=user_headers)
expect_status(f"{api_base}/api/v1/registry/authz", 401, headers=merged_headers(registry_basic_headers, {"X-Forwarded-Uri": f"/v2/{team_slug}/demo/manifests/latest"}))
expect_json(f"{api_base}/api/v1/user/api-keys/{quote_segment(user_key_id)}", method="DELETE", headers=user_headers)
