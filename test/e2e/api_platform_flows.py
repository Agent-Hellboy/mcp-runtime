...
expect_status(f"{api_base}/auth/login", 401, method="POST", body={"email": f"missing-{suffix}@mcpruntime.org", "password": "wrong-password"})
...
expect_status(f"{api_base}/auth/signup", 400, method="POST", body={"email": f"bad-role-{suffix}@mcpruntime.org", "password": test_user_password, "role": "root"})
expect_status(f"{api_base}/auth/signup", 403, method="POST", body={"email": f"admin-denied-{suffix}@mcpruntime.org", "password": test_user_password, "role": "admin"})

admin_login = expect_json(f"{api_base}/auth/login", method="POST", body={"email": platform_admin_email, "password": platform_admin_password})
...
signup = expect_json(f"{api_base}/auth/signup", status=201, method="POST", body={"email": signup_email, "password": test_user_password})
...
created_user_key = expect_json(f"{api_base}/user/api-keys", method="POST", headers=user_headers, body={"name": f"e2e-key-{suffix}"})
...
created_credential = expect_json(f"{api_base}/user/registry-credentials", status=201, method="POST", headers=user_headers, body={"name": f"e2e-registry-{suffix}"})
...
team_created = expect_json(f"{api_base}/runtime/teams", method="POST", headers=admin_headers, body={"slug": team_slug, "name": f"E2E API {suffix}"})
...
expect_json(f"{api_base}/runtime/teams/{quote_segment(team_slug)}", headers=admin_headers)
expect_json(f"{api_base}/runtime/teams/{quote_segment(team_slug)}/members", headers=admin_headers)
...
team_user = expect_json(f"{api_base}/users", status=201, method="POST", headers=admin_headers, body={"email": team_user_email, "password": test_user_password, "role": "user"})
...
expect_json(f"{api_base}/runtime/teams/{quote_segment(team_slug)}/members/{quote_segment(team_user_id)}", method="DELETE", headers=admin_headers)
...
expect_json(f"{api_base}/user/activity/image-publish", status=202, method="POST", headers=user_headers, body={
...
server_item = expect_json(f"{api_base}/runtime/servers/mcp-servers/{quote_segment(server_name)}", headers=admin_key_headers)
...
server_events = expect_json(f"{api_base}/runtime/server-events?namespace=mcp-servers&server={urllib.parse.quote(server_name)}&limit=5", headers=admin_key_headers)
...
expect_json(f"{api_base}/runtime/grants", method="POST", headers=admin_key_headers, body={
...
expect_json(f"{api_base}/runtime/sessions", method="POST", headers=admin_key_headers, body={
...
expect_json(f"{api_base}/runtime/grants/mcp-servers/{quote_segment(api_runtime_grant)}", headers=admin_key_headers)
expect_json(f"{api_base}/runtime/sessions/mcp-servers/{quote_segment(api_runtime_session)}", headers=admin_key_headers)
expect_json(f"{api_base}/runtime/sessions/mcp-servers/{quote_segment(api_runtime_session)}", method="DELETE", headers=admin_key_headers)
expect_json(f"{api_base}/runtime/grants/mcp-servers/{quote_segment(api_runtime_grant)}", method="DELETE", headers=admin_key_headers)

temp_server_name = f"e2e-api-server-{suffix}"
temp_server = expect_json(f"{api_base}/runtime/servers", method="POST", headers=admin_headers, body={
...
expect_json(f"{api_base}/runtime/servers/mcp-servers/{quote_segment(temp_server_name)}", headers=admin_headers)
expect_json(f"{api_base}/runtime/servers/mcp-servers/{quote_segment(temp_server_name)}", method="DELETE", headers=admin_headers)

deployment_name = f"e2e-api-deploy-{suffix}"
expect_json(f"{api_base}/deployments?namespace={quote_segment(team_namespace)}", headers=admin_headers)
deployment = expect_json_retry(f"{api_base}/deployments", method="POST", headers=admin_headers, body={
...
expect_json(f"{api_base}/admin/deployments?namespace={quote_segment(team_namespace)}", headers=admin_headers)
expect_json(f"{api_base}/deployments/{quote_segment(team_namespace)}/{quote_segment(deployment_name)}", method="DELETE", headers=admin_headers)
...
admin_operations = expect_json(f"{api_base}/admin/operations?limit=5", headers=admin_headers)
