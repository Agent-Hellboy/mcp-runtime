...
the VM at `/var/lib/mcp-runtime-e2e-backup/platform-runtime`. The next run
restores that snapshot before setup, preserving issued TLS secrets, platform
credentials, authorization-server secrets, and OIDC configuration without
requesting duplicate ACME certificates. This VM-side directory is the source
of truth; uploaded GitHub artifacts contain diagnostics only and are not used
as the certificate backup.
