...
`--mtls-cluster-issuer` and the adapter-enrollment stage can issue a real
session-bound client certificate. Set it to an empty value to skip that path.
...
| `adapter-enrollment` | `adapter enroll` issues a session-bound SPIFFE client certificate that verifies against the returned CA; an ungranted agent is refused a session (skipped when the ref has no `adapter enroll` or no mTLS issuer) |
