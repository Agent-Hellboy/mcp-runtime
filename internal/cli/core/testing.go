...
func NewTestKubectlClient(exec Executor) *KubectlClient {
	return &KubectlClient{exec: exec, validators: nil}
}

// NewTestKubectlClientWithValidators returns a KubectlClient for tests using the
// given validator list (or nil for none).
func NewTestKubectlClientWithValidators(exec Executor, validators []ExecValidator) *KubectlClient {
	return &KubectlClient{exec: exec, validators: validators}
}
